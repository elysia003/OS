...
class PT{
	...
	PT(int rn,int pn):romNum(rn),pagNum(pn),time(99999),state(false),visit(0){
...
class Quick_LRU:public Pross
{
	...
	pagNode *head;
	pagNode *end;
	Quick_LRU(int ps,int PN):Pross(ps)
	{
		PFNUM = PN;
		head = new pagNode;
		end = new pagNode;
